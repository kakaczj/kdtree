  ...
  , param_radius_ (::flann::SearchParams (-1, epsilon_, sorted))
{
  if (!std::is_same<std::size_t, pcl::index_t>::value) {
    ...
    if (std::is_same<int, pcl::index_t>::value) {
        LOG(INFO) << message; // since this has been the default behavior till PCL 1.12
    }
    else {
        LOG(WARNING) << message;
    }
...
pcl::KdTreeFLANN<PointT, Dist>::setInputCloud (const PointCloudConstPtr &cloud, const IndicesConstPtr &indices)
{
  ...
  if (!input_)
  {
      LOG(ERROR) << ("[pcl::KdTreeFLANN::setInputCloud] Invalid input!\n");
    return;
  }
  if (indices != nullptr)
  {
  ...
  {
      LOG(ERROR)<< ("[pcl::KdTreeFLANN::setInputCloud] Cannot create a KDTree with an empty input cloud!\n");
    return;
  }
